...
async def setup_settings(settings):
  try:
    ...
    patient = ChainlitAssistantAgent(
        ...
        system_message=sys_msgPat + "First, you will meet the nurse and she'll take you to the doctor's room. Then, you'll talk to the doctor."
    )
    nurse = ChainlitAssistantAgent(
        name="Nurse", llm_config=llm_config,
        system_message="You are the nurse in a hospital, your job is to meet a patient and tell them to talk to the doctor for their appointment. When the patient meets the doctor, you won't take part anymore in the conversation until the final decision of the doctor is taken. In this moment you will say goodbye to the patient and you will accompany him to the exit."
    )
    user_doctor = ChainlitUserProxyAgent(
        name="Doctor",
        human_input_mode="NEVER",
        llm_config=llm_config,
        system_message=sys_msgDct + "You will meet the nurse who will take the patient to your room where you're going to attend the patient's problems. After meeting the nurse you will be fully focused on the patient and you will call the nurse just when you communicate your final decision to the patient's diagnose.",
