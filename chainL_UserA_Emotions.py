...
def getEmotion(emotions):    # Continuing of the phrase: Your behaviour should be ...
    match emotions:
        case "Default":
...
async def setup_settings(settings):
  try:
    ...
    emotionsForPat = getEmotion(settings["Emotions"])
    txtForPatient = ""
    sys_msgPat = ""
    match settings["DataExtract"]:
        case "chargeSpecificFileICC":
            txtForPatient = chargeSpecificFile(1, "out_hce_insuf272_gpt-4_sum.txt",txtForPatient)
        case "chargeSpecificFileNOICC":
            txtForPatient = chargeSpecificFile(2, "out_hce_no_insuf218_gpt-4_sum.txt",txtForPatient)
